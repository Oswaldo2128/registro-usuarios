...
import mx.conacyt.crip.rizoma.registro.service.IdRizomaService;
 ...
 * <p>
 * This class accesses the {@link mx.conacyt.crip.rizoma.registro.domain.User}
...
public class UserResource {
	...
	private static final List<String> ALLOWED_ORDERED_PROPERTIES = Collections
			.unmodifiableList(Arrays.asList("id", "login", "firstName", "lastName", "id_rizoma", "email", "activated",
	...
	@PostMapping("/validateIdRizoma")
	public Mono<Boolean> validateIdRizoma(@RequestParam String idr) {
		Mono<User> usuario = userRepository.findOneByIdRizoma(idr);

		Mono<Boolean> flag = usuario.map(p -> {
			System.out.println(p.getIdRizoma());
			return true;
	...
	@PostMapping("/createIdRizoma")
	public void createIdRizoma(@RequestParam String nombre, String ap, String am) {

		String idr = IdRizomaService.crearIdr(nombre, ap).toLowerCase();
		System.out.println(idr);
		System.out.println(validateIdRizoma(idr));
	...
	public Mono<ResponseEntity<User>> createUser(@Valid @RequestBody AdminUserDTO userDTO) {
		...
		}).hasElement().flatMap(emailExists -> {
			if (Boolean.TRUE.equals(emailExists)) {
				return Mono.error(new EmailAlreadyUsedException());
			}
			return userService.createUser(userDTO);
		}).doOnSuccess(mailService::sendCreationEmail).map(user -> {
			try {
	...
	public Mono<ResponseEntity<AdminUserDTO>> updateUser(@Valid @RequestBody AdminUserDTO userDTO) {
		...
		return userRepository.findOneByEmailIgnoreCase(userDTO.getEmail())
				.filter(user -> !user.getId().equals(userDTO.getId())).hasElement().flatMap(emailExists -> {
					if (Boolean.TRUE.equals(emailExists)) {
						return Mono.error(new EmailAlreadyUsedException());
					}
	...
	public Mono<ResponseEntity<Void>> deleteUser(@PathVariable @Pattern(regexp = Constants.LOGIN_REGEX) String login) {
		...
		return userService.deleteUser(login).then(Mono.just(ResponseEntity.noContent()
