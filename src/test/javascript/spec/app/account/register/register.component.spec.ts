...
describe.skip('Register Component', () => {
  ...
  const filledRegisterAccount = { email: 'jhi@pster.net', langKey: 'es', login: 'jhi', password: 'jhipster', curp: 'CURP' };
  ...
  it('should register when password match', async () => {
    ...
    register.confirmEmail = filledRegisterAccount.email;
    register.register();
    await register.$nextTick();

    expect(
      axiosStub.post.calledWith('api/register', { email: 'jhi@pster.net', langKey: 'es', login: 'jhi', password: 'jhipster', curp: 'CURP' })
    ).toBeTruthy();
